...
const NOTES = {
 ...
 "第三步：参数怎么定": "回答参数怎么调这个核心问题。LLM 独立给出候选参数和方向，确定性搜索再独立建立内部参考。LLM 不负责生成搜索参考，这也是建议能够被自动判分的基础。",
 "第四步：怎么判断 LLM 提得对不对":
...
// ================= 3 整体做法：四步（原生流程图） =================
{
  ...
  const steps = [
    ...
    ["第四步", "核验它的建议", "跟有界搜索参考比", "regret 判分", WARN],
  ];
...
// ================= 8 第四步：怎么判分 =================
{
  ...
  bullets(s, [
    { t: "regret = 有界搜索参考分 − LLM 提议分", big: true, bold: true, color: ACCENT, gap: 9 },
